	...
	for (int rho = 0; rho < cells; rho++) {
		...
		*/
		...
		try{
			...
			*/			
			celll.magnetic_field = 0;//temporary removed: sqrt(pow(coreprof.toroid_field.b0 * coreprof.toroid_field.r0 / coreprof.rho_tor(rho),2) + pow(coreprof.profiles1d.bpol.value(rho),2)) ;
