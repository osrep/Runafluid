...
void fire(ItmNs::Itm::distribution &distribution, ItmNs::Itm::coreprof &coreprof, ItmNs::Itm::coreimpur &coreimpur, ItmNs::Itm::temporary &distribution_temp/*, ItmNs::Itm::temporaryArray &distribution_tempA*/) {
	...
	//std::cerr << "Length of distri_vec: \t"<< N_volume << std::endl << std::endl;
		...
		for (int i = 0; i < N; ++i){
			...
			//! Filling up runaway density and current from the input distribution
				distribution.distri_vec(0).profiles_1d.state.dens(i) = distribution_in.distri_vec(0).profiles_1d.state.dens(i);
				distribution.distri_vec(0).profiles_1d.state.current(i) = distribution_in.distri_vec(0).profiles_1d.state.current(i);
		...
		/*if (dist_initialised & flag==7){
			...
			for (int i = 0; i < N; ++i){
				if (i < N_rho_tor){
					distribution.distri_vec(0).profiles_1d.geometry.rho_tor(i) = distribution_in.distri_vec(0).profiles_1d.geometry.rho_tor(i);
				}
		
				if (i < N_rho_tor_norm){
					distribution.distri_vec(0).profiles_1d.geometry.rho_tor_norm(i) = distribution_in.distri_vec(0).profiles_1d.geometry.rho_tor_norm(i);
				}			
			}
